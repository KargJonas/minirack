...
static const int PIN_DOUT  = 39;  /* MISO, input-only */
static const int PIN_CLKIN = 14;
static const int PIN_RESET = 15;  /* shared SYNC/nRESET, active low */
static const int PIN_DRDY  = 35;  /* ADC1 only, input-only */

static const int ADC_CS[3] = {4, 17, 2};   /* ADC1, ADC2, ADC3 */

static const int CLKIN_HZ     = 4000000;
static const int CLKIN_LEDC_CH = 0;
...
static const SPISettings ADC_SPI_CFG(1000000, MSBFIRST, SPI_MODE1);
...
static uint16_t adcXferFrame(int csPin, uint16_t firstWord)
{
...
{
    adcXferFrame(csPin, OPCODE_RREG | ((uint16_t)address << 7));  /* frame 1 */
    delayMicroseconds(5);
    return adcXferFrame(csPin, OPCODE_NULL);                       /* frame 2 */
}

/* Hold the shared SYNC/nRESET low long enough to force a RESET (>2048 tCLKIN,
 * ~512 us at 4 MHz), then let all three chips come back up. */
...
/* Read ID/STATUS/MODE from every ADC and judge liveness. A 2-channel
 * ADS131M02 reports ID high byte 0x22 (reserved=0x2, CHANCNT=0x2). */
String adcTestJson()
{
    String out = "{\"clkin_hz\":";
    out += CLKIN_HZ;
...
void adcTestSerial()
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        uint16_t mode   = adcReadRegister(ADC_CS[i], REG_MODE);
        bool ok = (id >> 8) == 0x22;
        Serial.printf("  ADC%d (CS=IO%d): ID=0x%04X STATUS=0x%04X MODE=0x%04X "
                      "-> %s\n",
