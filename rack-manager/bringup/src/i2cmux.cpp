...
/* Fast-mode; see the bus-capacitance note in gpioexp.cpp. */
...
String i2cMuxTestJson()
{
    ...
    for (uint8_t ch = 0; ch < 8; ch++) {
        ...
        for (uint8_t addr = 0x08; addr <= 0x77; addr++) {
            if (addr == MUX_ADDR) continue;  // the mux itself acks on every channel
        ...
        if (ch == 0) {   // identify the known sensor board
            uint8_t id = 0;
            bool bmpLo = i2cReadReg(BMP280_ADDR_LO, BMP280_REG_ID, &id) && id == BMP280_CHIP_ID;
            
            uint8_t idHi = 0;
            bool bmpHi = i2cReadReg(BMP280_ADDR_HI, BMP280_REG_ID, &idHi) && idHi == BMP280_CHIP_ID;
...
String bmp280DebugJson()
{
    ...
    /**
     * The AHT20 is swept alongside the BMP280 rather than probed once at the
     * current clock: the bus runs at 400 kHz now, and "does this sensor answer
     * at 100 kHz but not 400" is exactly the question a speed change raises.
     */
    const uint32_t speeds[] = {100000, 50000, 400000};
    const uint8_t  addrs[]  = {BMP280_ADDR_LO, BMP280_ADDR_HI, AHT20_ADDR};
    bool first = true;
